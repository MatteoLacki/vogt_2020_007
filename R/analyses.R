...
get_pvals = function(comp, Rw, test, ratio){
  comp = c('accession',comp)
  Rw_comp = Rw[,..comp]
  colnames(Rw_comp) = c('accession','A','B')
  res = Rw_comp[,.(pval = test(A,B),
                   AoverB = ratio(A,B)),
                by=.(accession)]
  res[,`:=`(pval_bon= p_adjust(pval, method = 'bonferroni'),
            pval_BH = p_adjust(pval, method = 'BH'),
            pval_BY = p_adjust(pval, method = 'BY'))]
  res
}
...
longify = function(r) dcast(r, accession~comp, value.var = c('pval_bon','pval_BY', 'pval_BH', 'AoverB'), sep=": ")[order(accession)]
rw1 = longify(r1)
...
r3$method = 'new'
rr = rbind(r1, r2, r3)
rr = dcast(rr, comp+accession~method, value.var = c('pval', 'pval_BY', 'pval_bon', 'AoverB'))

with(rr, plot(pval_old, pval_new))
